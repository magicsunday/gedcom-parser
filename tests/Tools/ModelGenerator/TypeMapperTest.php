...
{
    /**
     * @return array<string, array{0: string, 1: string|null, 2: string, 3: string, 4: string, 5: string}>
     */
    public static function leafProvider(): array
    {
        ...
        return [
            // tag,   payload,              cardinality, phpType,      docType,        default
            'single string'  => ['PAGE', 'http://www.w3.org/2001/XMLSchema#string', '{0:1}', '?string', 'string|null', 'null'],
            'single enum'    => ['QUAY', $enum, '{0:1}', '?string', 'string|null', 'null'],
            'single date'    => ['DATE', $date, '{0:1}', '?DateValue', 'DateValue|null', 'null'],
            'single place'   => ['PLAC', $place, '{0:1}', '?PlaceValue', 'PlaceValue|null', 'null'],
            'pointer single' => ['SUBM', '@<https://gedcom.io/terms/v7/record-SUBM>@', '{0:1}', '?string', 'string|null', 'null'],
            'list string'    => ['REFN', 'http://www.w3.org/2001/XMLSchema#string', '{0:M}', 'array', 'list<string>', '[]'],
            'list date'      => ['DATE', $date, '{0:M}', 'array', 'list<DateValue>', '[]'],
        ];
    }

    /**
     * A leaf substructure maps to the expected PHP type, PHPDoc type and default per its payload and
     * cardinality.
     */
