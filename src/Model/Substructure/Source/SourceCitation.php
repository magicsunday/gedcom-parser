 ...
 *
 * It models the pointer citation's cross-reference, `PAGE`, `QUAY`, inline notes and the `DATA`
 * substructure (a typed {@see SourceCitationData}). The citation's `EVEN` substructure, the GEDCOM
 * 5.5.1 inline source-description text, and source citations carried by structures other than an
 * event are not yet modelled: as schema-recognised-but-unmodelled substructures they are currently
 * dropped (deferred to a later increment of the typed-model roll-out), NOT preserved on
 * {@see $unknown} — which captures only tags the schema does not permit at their position.
 *
